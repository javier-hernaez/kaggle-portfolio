...
1. PRICE-AWARE SELLING (Anti-Crash Arbitrage):
   - Reads obs["market"]["prices"] directly.
   - Sells in large batches (6-10) when market price is high.
   - Sells in small batches (1-2) when market price is low, allowing town shops
     (Ice Cream Shop, Brunch Spot, Bakery, Yarn Store) to drain town stock and restore prices.
2. BALANCED LIVESTOCK EXPANSION:
   - Smoothly alternates Cow and Sheep purchases up to 10 Cows + 4 Sheep (14 total).
   - Unlocks maximum wool ($200+) and milk ($160+) yield across all seeds.
3. MELON FERTILIZER MULTIPLIER:
...
class GrandmasterAgentV7:
    ...
    def select_crop(self, day, remaining_days, money, strawberry_count):
        ...
        # Phase 2: Days 5-13 -> STRAWBERRY SUPER-ENGINE (up to 40)
        if 5 <= day <= 13:
        ...
        # Phase 3: Days 14-25 -> Wheat for animal feed
        if 14 <= day <= 25:
            if money >= CROPS["WHEAT"]["seed"]:
                return "WHEAT"
            return None
    ...
    def plan_unit_turn(self, unit_idx, unit_pos, role, farm, private, day, hour, step, claimed_tiles, shared_state):
        ...
        # --- 1. PRIORITY SHED INTERACTION (When Adjacent to Shed) ---
        if (ux, uy) in SHED_ADJACENT:
            ...
            # Agriculture: pick up fertilizer for melons if available
    ...
    def act(self, obs):
        ...
        # ==========================================
        # 2. PRICE-AWARE MARKET SALES (Anti-Crash Slicing):
        # Sells large batches when price is healthy; throttles when depressed
        # ==========================================
        ...
        for prod, base_slice in sell_schedule:
            ...
            # If market price is severely depressed (< 70% of base), throttle batch
            if not is_endgame_liquidation and curr_price < base_p * 0.70:
                effective_slice = max(1, base_slice // 2)
            elif curr_price >= base_p * 1.10:
        ...
        # ==========================================
        # 5. BALANCED ANIMAL PURCHASES (Up to Day 11):
        # Alternates Cows and Sheep up to 10 Cows + 4 Sheep (14 total)
        # ==========================================
        total_capacity = len(PASTURE_LOCATIONS)
        if (total_animals + animals_in_shed) < total_capacity and day <= 11 and len(market_orders) < 9:
            # Alternating priority: keep sheep at roughly 1 per 2.5 cows
            if total_sheep < 4 and (total_cows >= total_sheep * 2 or total_cows >= 10) and money >= 600:
...
_grandmaster_agent = GrandmasterAgentV7()
